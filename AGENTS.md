...
- 最初に今回の確認対象が `ローカル作業ツリー`、`直近コミット`、`公開 repo / 既定ブランチ` のどれかを明示します。
- まず変更が現行フェーズスコープ内か確認します。
- 影響レイヤーを UI、Data Access、DB/RLS/RPC、Auth、Deploy、Docs、Test のいずれかに分類します。
- 正本ファイルと派生文書の従属関係を先に固定し、正本 → 関連文書 → 作業記録の順で更新します。
...
- 軽微変更では、`docs/templates/worklog.md` の「全変更共通の必須項目」を最小セットとして記録し、危険変更または大きなコード変更に当たる場合だけ追加項目を必須にします。
- worklogやPR本文は記録であり、正本ファイル本文、検索結果、差分、実行コマンド、GitHub上のコミット識別情報より先に根拠として扱いません。
...
軽微変更では、変更種別に応じた最低限の確認と最小記録で進めます。ただし、GitHub反映とCI要否判断は省略しません。

変更がある場合は、docs-only、設定変更、小修正、コード変更を問わず、完了報告前に必ず push して GitHub 上の反映ブランチとコミット識別情報を確認します。CI確認は毎回必須ではありませんが、大きなコード変更、CI/workflow/検証script変更、deploy経路変更、または依頼者が求めた変更では実施し、不要な場合も要否判断を記録します。
