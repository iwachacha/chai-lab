...
- 番号解釈: repo内の正は、`M2-01 = 検証/証跡土台整理`、`M2-02 = research_lines の最初の end-to-end DB slice` とする。M2-01完了後に次に着手すべき実装タスクは M2-02 と読む。M2-02へは、M2-01で再利用可能な非本番検証手順、証跡項目、未実施時の停止条件が固定されるまで進めない。
...
- 次へ進む条件: M2-09の4テーブル横断検証、M2-10の研究ラインData Access、M2-11のDB/RLS AI自己監査、M2-12の研究ライン集計付きData Access、M2-13のL1/L2研究ラインUIが完了している。未通過ならM3に進まない。
- 自己監査要否: 必須。
...
- 停止条件メモ: 次の項目は、M2-01完了後も未解消ならM2-02へ進めない条件として残す。
- 非本番検証方式未実証
- actor A/B/anonの切替方法未固定
- M2-01の補助証跡はあるが実slice適用は未着手
- slice固有SQLの具体化は未実施
- 次タスクへ渡す成果物: 検証/証跡チェックリスト
- 想定リスク: DDL先行で検証項目が後追いになる
- スコープ逸脱防止メモ: 実Supabase接続や本番向け設定を始めない。

#### M2-02 `research_lines` 最初の end-to-end DB slice
...
- 着手条件: M2-01完了。非本番検証方式未実証、actor A/B/anonの切替方法未固定、M2-01の補助証跡はあるが実slice適用は未着手のいずれかが残る場合は着手しない。
...
3. L1/L2研究ラインUI: M2-13着手はM2-12完了後
4. T1: M3-04後
