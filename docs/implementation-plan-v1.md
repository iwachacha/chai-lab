...
- 番号解釈: repo内の正は、`M2-01 = 検証/証跡土台整理`、`M2-02 = research_lines end-to-end DB slice` とする。M2-02へは、M2-01で再利用可能な非本番検証手順、証跡項目、未実施時の停止条件が固定されるまで進めない。
...
- 停止条件メモ: 非本番検証方式そのものの実証、actor A/B/anonの切替方法、slice固有SQLの具体化は未実施として残す。これらはM2-01の成果物から消さず、M2-02で適用時に引き継ぐ。
