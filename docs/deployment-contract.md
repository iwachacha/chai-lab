...
| Node.js version | プロジェクトで固定したLTS系を使用する |
...
2. `out` が生成される。
3. API Routes、Server Actions、Pages Functionsが存在しない。
4. `service_role` キーがフロントエンド環境変数に含まれていない。
5. Supabase Auth Redirect URLが対象環境に登録されている。
6. `/auth/callback/` が静的ページとして生成される。
7. ID付き画面がクエリパラメータ方式で動作する。
8. 未認証時に業務画面から認証画面へ誘導される。
9. RLSにより他ユーザーのIDを指定してもデータが表示されない。
