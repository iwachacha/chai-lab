...
- 今回の判断根拠: `ローカル作業ツリー`
- 公開 repo / 既定ブランチ確認: 作業開始時に `git fetch origin main` を実行し、`HEAD` と `origin/main` が `39093a4b1e1f6271d6d99aa3998f195590a27b77` で一致していることを確認した
...
- `supabase/verification/runs/2026-04-22-trials-minimum-slice-verification.sql`: 再実行可能な非本番DB/RLS/RPC検証SQLを追加
...
| `git fetch origin main`                                                                                                                                                                                  | 最新 `origin/main` 取得            | 成功。開始時 `HEAD` / `origin/main` は `39093a4b1e1f6271d6d99aa3998f195590a27b77`               |
...
| `npx -y -p @electric-sql/pglite node supabase/verification/scripts/run-pglite-verification.mjs ...`                                                                                                      | DockerなしDB/RLS/RPC検証の試行     | 失敗。`@electric-sql/pglite` の一時packageを既存ESM runnerから解決できず `ERR_MODULE_NOT_FOUND` |
...
- worklogに記録した成立済み事項:
  - DB/RLS/RPC危険変更の対象、権限境界、grant/revoke、未実施検証
  - 軽量ローカル検証とscope混入確認
  - PGlite検証を実行しようとしたが一時package解決で止まった事実
- あえて未解消として残した事項:
  - `clone_trial`、スター、下書き、日付範囲検索、T3フィルタは今回広げていない
  - Supabase実DB / local DBによるRLS実行検証は未完了
...
| ------------------------------------------------ | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ---------------------------------------------------------------------- |
| Supabase実DB / local DBでのRLS/RPC実行検証       | 低負荷方針でDocker常用を避け、PGlite実行も一時package解決で失敗したため      | 再実行可能SQL `supabase/verification/runs/2026-04-22-trials-minimum-slice-verification.sql`、migration目視、direct CRUD検索、Data Access test | 実Postgres/Supabase runtime上のgrant/RLS/RPC挙動差異は残る | 実DBへmigration適用、本番deploy、M4以降のRPC依存機能を完了扱いにする前 |
...
- direct CRUD拒否確認:
  ...
  - SQL検証ファイルにdirect insert/delete拒否probeを用意
