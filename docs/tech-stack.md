...
**作成日:** 2026-04-18
**v1方針:** v1では候補を広げず、実装再現性と保守性を優先して採用技術を固定する。
...
| スタイル | Tailwind CSS |
| UI部品 | Radix UI primitivesの必要最小限 |
...
### 3.5 Tailwind CSS + Radix UI primitives

デザイントークンを明示し、Codexが既存のスタイルに従いやすい構成にする。UI部品は必要最小限に留め、独自スタイルの乱立を避ける。

Radix UIは、Dialog、Dropdown Menu、Popover、Toastなど、アクセシビリティ要件があるプリミティブに限って採用してよい。通常のフォーム、カード、ボタン、リストはReactとTailwind CSSで実装する。Headless UIとの併用は避ける。
...
Radix UI内の新しいパッケージを追加する場合も、ライブラリ追加と同じ扱いにする。1つのUIのために複数のプリミティブをまとめて追加しない。

Node.jsのメジャーバージョンは、最初の実装PRで `.nvmrc` と `package.json` の `engines.node` に固定する。Cloudflare Pagesの設定も同じメジャーに合わせる。
...
v1の技術スタックは、機能を増やすためではなく、非公開の研究ログを安全に実装するために選定する。Next.js静的アプリ、npm、Supabase Auth、PostgreSQL、RLS、React Hook Form、Zod、TanStack Query、Tailwind CSS、必要最小限のRadix UI primitives、Playwrightを標準とし、それ以外は原則追加しない。
