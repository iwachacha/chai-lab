...
採用構成は **方針文書 + workflow + rules** とする。SKILLは作らない。
...
| SKILL | セッションをまたいだ再利用には強いが、この内容はリポジトリ固有で、外部再利用性は低い。保守単位も増える。 | 不採用 |
| 方針文書 + workflow + rules | 原則、手順、実装時の強制参照を分けられる。危険領域にも効き、過剰な仕組み化を避けられる。 | 採用 |

## 4. SKILLを作らない理由

`/SKILL.md` は作らない。理由は次のとおりである。
...
- `docs/agent-workflow.md` と `docs/codex-execution-rules.md` で、実装時の参照性と運用上の強さは足りる。

SKILL化を検討する条件は、将来、同じAI自律判断モデルを複数リポジトリへ移植し、リポジトリ固有のv1契約を切り離せる場合に限る。
...
| SKILL | 再利用性よりプロジェクト固有性が高く、保守コストに見合わない。 |
...
- SKILL化は、複数リポジトリで同じ運用を再利用する必要が出るまで行わない。
