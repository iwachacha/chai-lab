...
**運用注記:** 本計画は `docs/autonomous-project-governance.md` のAI自律判断モデルで運用する。旧来の `human review`、`human reviewer`、`project owner承認` は人間承認待ちではなく、AI自己監査ゲートまたは限定的人間確認として扱う。M1は静的基盤まで自律的に開始可とし、実Supabase project、Production URL、secret、Preview/Production接続、本番データ、外部サービス契約に触れる範囲だけ人間確認で止める。M0の詳細は `docs/m0-readiness-gate-reframed.md` と `docs/m0-open-questions-reframed.md` を正とする。

## 1. 改訂版の目的と前提

この文書は、旧実装計画と監査結果を統合し、安全側に改訂した最新版の実装計画書である。旧計画は、v1スコープ意識はある一方で、DB/RLS/RPCの検証ゲート不足、巨大タスク、危険領域の監査不足、AppResult/AppErrorの曖昧さ、受け入れ基準との対応不足があり、そのまま実装を始めてよい状態ではなかった。
...
Q-01からQ-10の詳細分類は `docs/m0-open-questions-reframed.md` を正とする。この計画では、次の分類で停止範囲を扱う。

| ID | 論点 | 分類 | 採用案 | 未達時に止める範囲 |
|---|---|---|---|---|
| Q-01 | Supabase local/preview/productionの分離、Auth Redirect URL、テストデータ方針 | 人間確認が必要 | 実Supabase project、Production URL、secret、本番データ、外部サービス接続だけ依頼者へ確認する。M1の静的骨格は仮envで進める。 | 実DB接続、Preview/Production接続、M8 deploy |
...
| Q-03 | DB/RLS/RPC自己監査運用 | AIが即時決定してよい | 旧来の人間承認ゲートをAI自己監査ゲートへ置換し、設計、影響範囲、権限、代替案、検証、記録をAIが満たす。 | 危険領域の完了扱いと依存UI |
| Q-04 | 研究ライン名重複の正規化範囲 | AIが即時決定してよい | v1は `trim` のみ正規化し、大文字小文字、全角半角、Unicode正規化は同一視しない。 | 研究ラインunique制約とData Access |
...
| Q-10 | 作業記録の保存先 | AIが即時決定してよい | 通常はPR本文、PRがない作業は `docs/governance-refactor-plan.md` または該当作業記録に追記する。 | 危険領域の監査記録が必要なタスク |
...
理由は、Critical 3件とMajor/Minorの監査指摘は計画上のタスク、ゲート、完了条件、対応表へ反映済みであり、Q-02、Q-03、Q-04、Q-06、Q-09、Q-10はAIが即時決定でき、Q-05とQ-07は安全側の仮決定で進められるためである。Q-01の実環境接続とQ-08の本番運用確認だけは限定的人間確認として扱う。
