...
**運用注記:** 判断主体とエスカレーション範囲は `docs/agent-relationship-governance.md` を優先し、実装前、危険変更前、停止時、エスカレーション時の手順は `docs/agent-workflow.md` に従う。Codexは依頼者に技術承認を求める前に、文書根拠、現行フェーズ整合、安全性、単純性、可逆性、監査可能性に基づいて推奨案を1つに絞り、自律的に進める。人間確認はスコープ変更、外部契約、本番データ、不可逆操作、純粋なプロダクト方針選択に限定する。worklogやPR本文は記録であり、正本ファイル本文、検索結果、差分、実行コマンド、GitHub上のコミット識別情報より先に根拠として扱わない。
...
変更規模ごとの完了運用は `docs/agent-workflow.md` の「変更規模の分類、記録密度、完了条件」に従う。軽微変更では worklog / PR本文の全変更共通の必須項目だけを最小セットとして残し、大きなコード変更または危険変更では追加項目まで残す。変更を伴う作業は、docs-only、設定変更、小修正、コード変更を問わず、完了報告前に必ずpushし、GitHub上の対象ブランチとコミット識別情報を確認する。CI確認は、大きなコード変更、CI/workflow/検証scriptやdeploy経路の変更、または依頼者が明示的に求めた場合に必須とする。それ以外では、CI未確認でもよいが、要否判断、未確認理由、代替確認、残リスクを最終報告または作業記録へ残す。
...
Codexの最終報告は、`docs/agent-workflow.md` の完了報告workflowに合わせて変更規模ごとに密度を分ける。
...
過度に長い説明ではなく、ユーザーが検証と次の判断をできる粒度で報告する。
...
- [ ] 変更は現行フェーズスコープ内である。
- [ ] 今回の確認対象を `ローカル作業ツリー`、`直近コミット`、`公開 repo / 既定ブランチ` の語で宣言した。
- [ ] 正本ファイルを特定した。
- [ ] 対象文書を読んだ。
- [ ] 既存の用語に沿っている。
...
- [ ] 新しいライブラリを勝手に追加していない。
- [ ] 範囲外機能を先回りしていない。
- [ ] 変更がある場合は、GitHub反映とCI要否判断まで完了条件に含める。
