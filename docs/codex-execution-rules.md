...
変更規模ごとの完了運用は `docs/agent-workflow.md` の「変更規模の分類と完了条件」に従う。変更を伴う作業は、docs-only、設定変更、小修正、コード変更を問わず、完了報告前に必ずpushし、GitHub上の対象ブランチとコミット識別情報を確認する。CI確認は、大きなコード変更、CI/workflow/検証scriptやdeploy経路の変更、または依頼者が明示的に求めた場合に必須とする。それ以外では、CI未確認でもよいが、要否判断、未確認理由、代替確認、残リスクを最終報告または作業記録へ残す。
